...
// CRITICAL: Block stride for overlap (1 cell overlap between adjacent blocks)
...
layout(set = 0, binding = 0, std140) uniform PushConstants {
    ...
    uvec4 blockStride;   // Block stride for overlap (7x7x7)
...
// Determine if this block "owns" a cell in the overlap region
bool blockOwnsCell(ivec3 globalCellPos, uvec3 blockOrigin) {
    // For cells in overlap regions, the block with the lowest
    // coordinates that can process it owns it
    
    // Which blocks could process this cell?
    ivec3 blockCoordForCell = globalCellPos / ivec3(BLOCK_STRIDE_X, BLOCK_STRIDE_Y, BLOCK_STRIDE_Z);
    uvec3 ourBlockCoord = blockOrigin / uvec3(BLOCK_STRIDE_X, BLOCK_STRIDE_Y, BLOCK_STRIDE_Z);
    
    // We own it if we're the lowest-coordinate block that contains it
    return all(equal(ivec3(ourBlockCoord), blockCoordForCell));
}
...
void main() {
    ...
    // CRITICAL: Use stride for block positioning to create overlap
