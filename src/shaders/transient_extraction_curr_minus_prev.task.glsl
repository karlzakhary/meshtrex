...
void main() {
    ...
    // Check if we're within the PVS_curr-prev count
    if (workgroupIndex >= pvsDifference.count * 2) {
        return;
    }
    
    // Get the block ID from PVS_curr-prev buffer
    uint blockListIndex = workgroupIndex / 2;
    uint halfIndex = workgroupIndex % 2;
    uint blockID = pvsDifference.blockIds[blockListIndex];
    ...
    if (threadID == 0) {
        ...
        if (workgroupIndex == 0 || blockID == 292) {
            debugPrintfEXT("Task shader pass2: block %d, half %d, occupied cells: %d, meshlets: %d", 
