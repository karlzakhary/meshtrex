...
// Maps an 8-bit cube index to a list of 12 vertex indices (0X1) needed to form triangles.
// Max 5 triangles (15 indices) per case. Each row MUST have exactly 16 entries.
// List terminated by X. More Xs pad rows with fewer than 5 triangles.
...
#define X 255
// triTable[caseIndex * 16 + i] gives the i-th vertex index (0X1) for the triangles,
// or X if fewer than 15 vertices are needed.
static const uint8_t triTable[256][16] =
...
static const uint8_t numVerticesTable[256] =
{
    0,
    3,
    3,
...
inline constexpr uint8_t uniqueTriTable[256][16] =
{
{X,X,X,X,X,X,X,X,X,X,X,X,X,X,X,X},
{0,8,3,X,X,X,X,X,X,X,X,X,X,X,X,X},
{0,1,9,X,X,X,X,X,X,X,X,X,X,X,X,X},
...
/*
  ...
  not 15
*/
inline constexpr uint8_t numUniqueVertsTable[256] =
{
0,
3,
3,
