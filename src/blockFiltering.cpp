...
uint32_t filterUnoccupiedBlocks(char **argv, const char *path)
{
    ...
    VulkanContext context(false); // Request mesh shading = false

    // Use the context via its getter methods
    ...
    std::vector<MinMaxResult> results = mapMinMaxImage(
        device, context.getMemoryProperties(), context.getCommandPool(), context.getQueue(),
