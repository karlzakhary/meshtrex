...
bool validateMeshletDescriptors(VulkanContext& context, const ExtractionOutput& extractionOutput) {
    std::cout << "\n--- Starting Meshlet Descriptor Validation ---" << std::endl;

    // --- 1. Read all necessary data from GPU buffers ---

    // Read the total counts generated by the extraction shader
    uint32_t totalMeshlets = mapCounterBuffer(context, extractionOutput.meshletDescriptorCountBuffer);
    uint32_t totalVertices = mapCounterBuffer(context, extractionOutput.vertexCountBuffer);
    uint32_t totalIndices = mapCounterBuffer(context, extractionOutput.indexCountBuffer);

    if (totalMeshlets == 0) {
        std::cout << "Validation successful: No meshlets were generated." << std::endl;
        return true;
    }

    std::cout << "Reading " << totalMeshlets << " meshlet descriptors from GPU..." << std::endl;
    std::cout << "Total generated vertices: " << totalVertices << std::endl;
    std::cout << "Total generated indices:  " << totalIndices << std::endl;

    // Read the entire array of descriptors
    std::vector<MeshletDescriptor> descriptors = mapMeshletDescriptorBuffer(context, extractionOutput.meshletDescriptorBuffer, totalMeshlets);
    if (descriptors.size() != totalMeshlets) {
        std::cerr << "ERROR: Failed to read back the correct number of meshlet descriptors." << std::endl;
        return false;
    }

    // --- 2. Perform Validation Checks ---

    uint32_t invalidDescriptorCount = 0;
    const uint32_t MAX_VERTS_PER_MESHLET = 64; // Match shader's max_vertices
    const uint32_t MAX_PRIMS_PER_MESHLET = 126; // Match shader's max_primitives

    for (uint32_t i = 0; i < totalMeshlets; ++i) {
        const auto& desc = descriptors[i];
        bool isValid = true;

        // Check 1: Logical consistency. A meshlet should have both vertices and primitives.
        if ((desc.vertexCount > 0 && desc.primitiveCount == 0) || (desc.vertexCount == 0 && desc.primitiveCount > 0)) {
            std::cerr << "ERROR [Descriptor " << i << "]: Inconsistent counts. "
                      << "Vertices: " << desc.vertexCount << ", Primitives: " << desc.primitiveCount << std::endl;
            isValid = false;
        }

        // Check 2: Vertex buffer access violation.
        // This is the most common cause of "exploding vertices".
        if (desc.vertexOffset + desc.vertexCount > totalVertices) {
            std::cerr << "ERROR [Descriptor " << i << "]: Vertex buffer out-of-bounds access. "
                      << "Starts at " << desc.vertexOffset << " with " << desc.vertexCount << " verts, "
                      << "but total vertices is only " << totalVertices << "." << std::endl;
            isValid = false;
        }

        // Check 3: Index buffer access violation.
        uint32_t indexAccessCount = desc.primitiveCount * 3;
        if (desc.indexOffset + indexAccessCount > totalIndices) {
            std::cerr << "ERROR [Descriptor " << i << "]: Index buffer out-of-bounds access. "
                      << "Starts at " << desc.indexOffset << " with " << indexAccessCount << " indices, "
                      << "but total indices is only " << totalIndices << "." << std::endl;
            isValid = false;
        }
        
        // Check 4: Sanity check against shader limits.
        if (desc.vertexCount > MAX_VERTS_PER_MESHLET) {
            std::cerr << "ERROR [Descriptor " << i << "]: Exceeds max vertices per meshlet. "
                      << "Has " << desc.vertexCount << ", limit is " << MAX_VERTS_PER_MESHLET << "." << std::endl;
            isValid = false;
        }
        if (desc.primitiveCount > MAX_PRIMS_PER_MESHLET) {
             std::cerr << "ERROR [Descriptor " << i << "]: Exceeds max primitives per meshlet. "
                      << "Has " << desc.primitiveCount << ", limit is " << MAX_PRIMS_PER_MESHLET << "." << std::endl;
            isValid = false;
        }
        ...
        if (!isValid) {
            ...
            // Print the garbage data for the invalid descriptor
            std::cout << "    --> Garbage Descriptor Data: "
                      << "vOffset=" << desc.vertexOffset << ", "
                      << "iOffset=" << desc.indexOffset << ", "
                      << "vCount=" << desc.vertexCount << ", "
                      << "pCount=" << desc.primitiveCount << std::endl;
        }
    }

    std::cout << "\n--- Validation Summary ---" << std::endl;
    if (invalidDescriptorCount == 0) {
        std::cout << "✅ Success! All " << totalMeshlets << " meshlet descriptors are valid." << std::endl;
        return true;
    } else {
        std::cout << "❌ Failure! Found " << invalidDescriptorCount << " invalid descriptors out of " << totalMeshlets << "." << std::endl;
        return false;
    }
}