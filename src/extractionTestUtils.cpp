...
// Validation function to check for subdivision issues
void validateSubdivisionResults(VulkanContext& context, ExtractionOutput& extractionResult) {
    std::cout << "\n=== Validating Subdivision Results ===" << std::endl;
    
    uint32_t actualMeshletCount = readCounterFromBuffer(context, extractionResult.meshletCountBuffer);
    uint32_t actualVertexCount = readCounterFromBuffer(context, extractionResult.globalVertexCountBuffer);
    uint32_t actualIndexCount = readCounterFromBuffer(context, extractionResult.globalIndexCountBuffer);
    
    std::cout << "Global counters:" << std::endl;
    std::cout << "  Meshlets: " << actualMeshletCount << std::endl;
    std::cout << "  Vertices: " << actualVertexCount << std::endl;
    std::cout << "  Indices: " << actualIndexCount << std::endl;
    
    if (actualMeshletCount == 0) {
        std::cout << "No meshlets to validate." << std::endl;
        return;
    }
    ...
    uint32_t totalDescVertices = 0;
    uint32_t totalDescIndices = 0;
    uint32_t invalidMeshlets = 0;
    uint32_t maxVertexIndex = 0;
    uint32_t minVertexIndex = UINT32_MAX;
    ...
    for (uint32_t i = 0; i < descriptors.size(); i++) {
        ...
        if (desc.vertexOffset + desc.vertexCount > actualVertexCount) {
            std::cout << "Invalid meshlet " << i << ": vertex range [" 
                      << desc.vertexOffset << "," << (desc.vertexOffset + desc.vertexCount) 
                      << ") exceeds total vertices " << actualVertexCount << std::endl;
            invalid = true;
        }
        
        if (desc.indexOffset + (desc.primitiveCount * 3) > actualIndexCount) {
            std::cout << "Invalid meshlet " << i << ": index range [" 
                      << desc.indexOffset << "," << (desc.indexOffset + desc.primitiveCount * 3) 
                      << ") exceeds total indices " << actualIndexCount << std::endl;
            invalid = true;
        }
        ...
        totalDescVertices += desc.vertexCount;
        totalDescIndices += desc.primitiveCount * 3;
        
        maxVertexIndex = std::max(maxVertexIndex, desc.vertexOffset + desc.vertexCount);
        minVertexIndex = std::min(minVertexIndex, desc.vertexOffset);
    }
    
    std::cout << "Validation results:" << std::endl;
    std::cout << "  Invalid meshlets: " << invalidMeshlets << "/" << descriptors.size() << std::endl;
    std::cout << "  Total vertices from descriptors: " << totalDescVertices << std::endl;
    std::cout << "  Total indices from descriptors: " << totalDescIndices << std::endl;
    std::cout << "  Vertex range used: [" << minVertexIndex << "," << maxVertexIndex << ")" << std::endl;
    std::cout << "  Vertex buffer utilization: " << (float)maxVertexIndex / actualVertexCount * 100.0f << "%" << std::endl;
    
    if (totalDescVertices > actualVertexCount) {
        std::cout << "ERROR: Descriptor vertices exceed global count!" << std::endl;
    }
    if (totalDescIndices > actualIndexCount) {
        std::cout << "ERROR: Descriptor indices exceed global count!" << std::endl;
    }
}
