    ...
    const char* filePath)
{
    ...
    if (totalGlobalVertices == 0 || totalGlobalTriangles == 0) {
        std::cout << "  No global vertices or triangles generated. OBJ file will be empty." << std::endl;
        std::ofstream outFile(filePath); // Create an empty file
        if (outFile.is_open()) {
            outFile << "# No geometry generated." << std::endl;
            outFile.close();
        }
        return;
    }
    ...
    if (globalVertices.empty() && totalGlobalVertices > 0) {
        std::cerr << "  Error: Failed to read global vertex data from GPU. Aborting OBJ write." << std::endl;
        return;
    }
    if (globalIndices.empty() && totalGlobalTriangles > 0) {
        std::cerr << "  Error: Failed to read global index data from GPU. Aborting OBJ write." << std::endl;
        return;
    }

    // At this point, globalVertices contains all unique vertices,
    // and globalIndices contains the indices (which are direct indices into globalVertices).
    // The concept of iterating meshlet descriptors to gather geometry is no longer the primary way.
    // The OBJ can be written directly from these two global, already deduplicated buffers.
    ...
    // but they are not needed to construct the geometry if using global V/I buffers.
    ...
    for (const auto& vertex : globalVertices) {
        outFile << "v " << vertex.position.x << " " << vertex.position.y << " " << vertex.position.z << std::endl;
        // Assuming normals are correctly populated in VertexData by the shader
        outFile << "vn " << vertex.normal.x << " " << vertex.normal.y << " " << vertex.normal.z << std::endl;
    }

    for (size_t i = 0; i < globalIndices.size(); i += 3) {
        if (i + 2 < globalIndices.size()) {
            // OBJ indices are 1-based. The globalIndices from shader are 0-based.
            uint32_t v0 = globalIndices[i];
            uint32_t v1 = globalIndices[i+1];
            uint32_t v2 = globalIndices[i+2];

            // Sanity check indices against the number of vertices read
            if (v0 >= totalGlobalVertices || v1 >= totalGlobalVertices || v2 >= totalGlobalVertices) {
                std::cerr << "  Error: OBJ Exporter found an out-of-bounds global index! "
                          << "v0=" << v0 << ", v1=" << v1 << ", v2=" << v2
                          << " (Total Global Vertices: " << totalGlobalVertices << "). Skipping face." << std::endl;
                continue;
            }
    ...
    outFile.close();
    std::cout << "Successfully wrote global vertex/index data to " << filePath << std::endl;
}
